...
var
    ...
    best_move_one_only = newSeqwith(3125, -1)
    first_b: set[int8]
...
proc get_best_move(A_n: seq[seq[int8]]): int8 =
    ...
    if len(best_move[tmp]) > 0:
        if move_choice_mode == 0:
            return int8(sample(best_move[tmp]))
        else:
            return int8(best_move_one_only[tmp])
    else:
        return -1
...
var space: set[int8] = {13, 3, 23, 12, 2, 22}

proc climb_hill_12(score: int, first_ans: seq[seq[string]]): (seq[seq[string]]) =
    move_choice_mode = 1
    ...
    var
        ...
        cnt = 0
        last_ans = first_ans
        can_move = newSeqwith(3125, newSeqOfCap[int](4))

    for i in 0..<3125:
        if len(best_move[i]) == 0:
            continue
        best_move_one_only[i] = sample(best_move[i])
        for j in 0..<5:
            if (i mod (5**(j+1)) div (5**j)) > 0:
                can_move[i].add(j)

    while true:
        if cputime() - START_TIME > 2.95:
            break
        var mode = sample([0, 0, 1, 1, 1, 2, 3, 3, 3])
        #削除
        if mode == 0:
            var v = sample(space)
            space.excl(v)
        ...
        #場所変更
            ...
            var
                v = rand(3124)
                bef = best_move_one_only[v]
                aft = sample(can_move[v])
            if bef == aft:
                continue
            best_move_one_only[v] = aft
            var
                (ok, ans) = solve(trans_space(space))
                score = get_score(ans)
            ...
            if ok and score < best_score:
                ...
                best_move_one_only[v] = bef

    return last_ans

proc climb_hill_1(score: int, first_ans: seq[seq[string]]): (seq[seq[string]]) =
    var
        all: seq[int8] = @[0, 1, 5, 6, 7, 8, 10, 11, 15, 16, 17, 18, 20, 21, 13, 3, 23, 12, 2, 22]
        space_not_use: set[int8] = {0, 1, 5, 6, 7, 8, 10, 11, 15, 16, 17, 18, 20, 21}
        # space_not_use: set[int8] = {0, 1, 5, 6, 10, 11, 15, 16, 20, 21, 13, 3, 23, 12, 2, 22}
...
proc climb_hill_2(score: int, first_ans: seq[seq[string]]): seq[seq[string]] =
    move_choice_mode = 1
    var
        best_score = score
        can_move = newSeqwith(3125, newSeqOfCap[int](4))
        ans = first_ans
        space = trans_space(space)
...
proc main() =
    first_set_up()
    calc_dp()
    var ans = climb_hill_12(1000, first_ans)
    # var ans = climb_hill_2(1000, first_ans)
    # ans = climb_hill_1(get_score(ans), ans)
    output(ans)
