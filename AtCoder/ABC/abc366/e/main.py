...
dis_x = [tmp]
r = -1
for x in range(-1999999, 2000001):
    while r + 1 < n and x_list[r + 1] <= x - 1:
        r += 1
    dis_x.append(dis_x[-1] + (r + 1) - (n - (r + 1)))
...
dis_y = [tmp]
r = -1
for y in range(-1999999, 2000001):
    while r + 1 < n and y_list[r + 1] <= y - 1:
        r += 1
    dis_y.append(dis_y[-1] + (r + 1) - (n - (r + 1)))
tmp = 0
ans = 0
dis_x.sort()
dis_y.sort()
...
r = -1
for i in dis_x[::-1]:
    while r + 1 < len(dis_y) and i + dis_y[r + 1] <= d:
        r += 1
    ans += r + 1
print(ans)
