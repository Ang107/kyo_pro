...
q = II()
cd = [[i - 1 for i in LMII()] for _ in range(q)]
cp = cd[:]
cd = get_unique(cd)
cd.sort()
ab.sort()
...
ans = defaultdict(int)
for p, q in cd:
    while idx < len(ab) and ab[idx][0] < p:
        bit.add(ab[idx][1], 1)
        idx += 1
    ans[(p, q)] += bit.sum(p, q + 1)

ab.sort(reverse=True, key=lambda x: x[1])
cd.sort(reverse=True, key=lambda x: x[1])
idx = 0
bit = BIT(2 * n)
for p, q in cd:
    while idx < len(ab) and q < ab[idx][1]:
        bit.add(ab[idx][0], 1)
        idx += 1
    ans[(p, q)] += bit.sum(p, q + 1)
for i, j in cp:
