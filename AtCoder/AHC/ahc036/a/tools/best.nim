...
proc check(input: Input, output: Output, p: seq[int], d: array[N, (int, int,
    ...
    for i in 0..<N:
        sum_dis += dis[i][0]
    echo "# sum_dis", sum_dis
    echo "# avr_dis", sum_dis / 600
    echo "# score", 2*(sum_dis + input.LB-1) // input.LB + 600
    ...
    for i in 0..<N:
        echo "# index", i
        echo "# all_edge", input.G[i]
        echo "# edge", g[i]
    A = compression_tree(p[0])
...
proc make_A(input: Input, output: var Output) =
    var
        ...
        # 距離、インデックス
        by_center = (INF64, -1)
        by_target: array[8, (int, int)]
        mid_by_target: array[8, int]
    ...
    #     if i & 1 == 0:
    #         var root = bfs(input, index, by_center[1])
    #         mid_by_target[i] = root[root.len()//2]
    #         for index in 0..<root.len():
    #             if not visited[root[index]]:
    #                 position.add(root[index])
    #             visited[root[index]] = true
    #             output.A.add(root[index])
    #     else:
    #         var root = bfs(input, by_center[1], index)
    #         mid_by_target[i] = root[root.len()//2]
    ...
    # var square = [(by_target[0][1], by_target[4][1], by_target[3][1]),
    #         (by_target[3][1], by_target[7][1], by_target[1][1]),
    #         (by_target[1][1], by_target[5][1], by_target[2][1]),
    #         (by_target[2][1], by_target[6][1], by_target[0][1]), ]
    # for i, (a, b, c) in square:
    #     var root = bfs(input, a, b)
    #     for index in 0..<root.len():
    #         if not visited[root[index]]:
    #             position.add(root[index])
    #         visited[root[index]] = true
    ...
    # (ok, A, d) = check(input, output, position, dis, is_need_area, visited)
    # dis = d
    ...
    # var cross: array[4, (int, int, int)]
    # cross = [(by_target[6][1], mid_by_target[0], by_target[4][1]),
    #         (by_target[4][1], mid_by_target[3], by_target[7][1]),
    #         (by_target[7][1], mid_by_target[1], by_target[5][1]),
    #         (by_target[5][1], mid_by_target[2], by_target[6][1]), ]

    # for i, (a, b, c) in cross:
    #     var root = bfs(input, a, b)
    #     for index in 0..<root.len():
    #         if not visited[root[index]]:
    #             position.add(root[index])
    ...
    # ここ改善できそう
    # var
    #     diss = newSeq[(int, int)](N)
    #     roots = newSeq[newSeqOfCap[int](20)](N)
    # for i in 0..<T:
    #     var
    #         s = input.TS[i]
    #         g = input.TS[i+1]
    ...
    #     roots[i] = root
    #     diss[i] = (root.len(), i)
    # diss.sort(proc (a, b: (int, int)): int = cmp(a[0], b[0]))
    # for (_, i) in diss:
