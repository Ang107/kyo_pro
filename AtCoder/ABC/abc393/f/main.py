...
n, q = MII()
seg = SegTree(lambda a, b: max(a, b), 0, n)
a = LMII()
pos = {j: i for i, j in enumerate(sorted(set(a)))}
sa = sorted(set(a))
qs = [LMII() + [i] for i in range(q)]
...
for r, x, i in qs:
    ...
    for j in range(now, r + 1):
        tmp = seg.prod(0, pos[a[j]])
        seg.set(pos[a[j]], tmp + 1)
    now = r
    # print(seg)
    ans[i] = seg.prod(0, bisect_right(sa, x))
for i in ans:
    print(i)
