...
op = lambda a, b: max((a[0], -a[1]), (b[0], -b[1]))
e = (-float("inf"), inf)
h, w, n = MII()
rc = [LMII() for _ in range(n)]
xy = [[] for _ in range(h)]
for r, c in rc:
    r -= 1
    c -= 1
...
for i in range(h):
    xy[i].sort()
st = SegTree(op, e, [(0, i) for i in range(w)])
ans = [[] for _ in range(h)]
for i in range(h):
    for j in xy[i]:
        st.set(j, (st.prod(0, j + 1)[0] + 1, j))
        ans[i].append((j, st.get(j)))
    # ans.append(st.all_prod()[1])
    # print([st.get(i) for i in range(w)])
print(ans)
ret = st.all_prod()[0]
