...
for _ in range(t):
    ...
    h = LMII()
    # hpの累積和
    h_acc = [0] + list(accumulate(h))
    # hpの累積和 <= 距離ならクリアできそう
    # 超過の最大値
    # over_max = [0]
    # for i, j in zip(h_acc[::-1], a[::-1]):
    #     over_max.append(max(over_max[-1], i - j))
    # over_max = over_max[::-1]
    v = []
    for i, j in zip(h_acc[1:], a):
        v.append(i - j)
    # 区間加算・区間最大値取得^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    for idx, i in enumerate(a):
        if st.prod(0, idx) > 0:
            continue
        # 爆弾の届く右端
        r = bisect_right(a, i + 2 * k) - 1

        # 倒せるモンスターの総HP
        ad = h_acc[r + 1] - h_acc[idx]
        # print(idx, i)
        # print(idx, r, i + 2 * k)
        # print(ad, st.prod(r + 1, n))
