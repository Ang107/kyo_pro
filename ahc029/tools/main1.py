...
def First_input():
    ...
    global N,M,K,T,L,cards,projects,projects_cospa,max_change_card,card_0_count,card_4_count,hosei,st,card_4_limit
    N,M,K,T = MII()
...
def Mid_input(bef_c):
    global N,M,K,T,cards,projects,projects_cospa,money,hosei,card_4_limit,st
    projects = [LMII() for _ in range(M)]
    money = II()
...
# def calc_hosei0():
#     global st
#     temp = clamp(card_0_count/Tarn,0.8,1)
#     temp = temp ** 0.1
#     st = 1.4*temp
...
def Choice_in(cards,add_cards,projects):
    ...
    next_r = projects[projects_cospa.index(max(projects_cospa))][1]
    ...
    for i,j,k in add_cards:
        if k <= money:
            if i == 0:
                ...
                if k == 0:
                    card_choice.append(st)
                else:
                    # print(f"#{pro_cospa_max*j*1.01**L/k,(pro_cospa_max*j*1.01**L/k + 0.5*pro_cospa_max)*2/3,pro_cospa_max}")

                    card_0_count += 1
                    if  money  > 400 * 2**L and  money - k + next_r < 300 * 2**L and  Tarn < 950:
                        # card_choice.append(j*1.01**(L+2)/k)
                        card_choice.append(0.95*j/k)
                    else:
                        card_choice.append(j*1.0/k)
                ...
                if   card_kind[2]+card_kind[3] < max_change_card and k == 0 :
                    card_choice.append((st+0.01))
                else:
                    card_choice.append(0)
            elif i == 3:
                change_card = All_bad(projects,3)
                if   card_kind[2]+card_kind[3] < max_change_card and k == 0 :
                    card_choice.append((st+0.02))
                else:
                    card_choice.append(0)
            elif i == 4 :
                    card_4_count += 1
                    if 20 > L and Tarn < 950 and  k < 2 ** L * (1000-Tarn) * 1.4 and k / 2**L <= card_4_limit:
...
def All_bad(projects,card):
    ...
    if card == 2:
        ...
        temp = math.log(max(cost_paf),2)
        # 
        # print(f"#{max(cost_paf) > 1.2*1.02**M,1-(1-calc_persent(0,0.5,temp)) ** M < 0.75}")
        # print(f"#{temp,1-(1-calc_persent(0,0.5,temp)) ** M}")
        # if 1 - calc_persent(0,0.5,temp) ** M <= 0.75:
...
def Pick_pro(projects):
    ...
    if Tarn < 900:
        ...
        for i,j in enumerate(projects):
            if j[0] <= (1000 -Tarn) * 2**L * 3 * card_0_count/Tarn + sum([j for i,j in cards]):
