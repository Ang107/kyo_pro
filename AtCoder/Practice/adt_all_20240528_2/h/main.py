...
n = II()
A = LMII()
tmp = {j:i for i,j in enumerate(sorted(set(A)))}
# 区間加算・区間和取得^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
st = LazySegTree(op,e,mapping,composition,id_,[0] * len(tmp))
ans = 0
for i in A:
    # pritn(st.prod(0,tmp[i]+1))
    ans += st.prod(0,tmp[i]+1)
    ans %= mod
    
    st.apply(0,len(tmp),2)
    st.set(tmp[i],st.get(tmp[i]) + 1)
    # print(st)
