...
let
    ...
    w = ii()
var
    c = newSeqOfCap[string](h)

for i in range(h):
    c.add(si())

proc bfs(): int =
    var deq: Deque[array[2, int]] = initDeque[array[2, int]]()
    deq.addLast([0, 0])
    var visited = newSeq[seq[int]](h)
    visited.fill(newSeq[int](w))
    visited[0][0] = 1
    result = 1
    # echo deq
    while len(deq) > 0:
        ...
        for ij in [[1, 0], [0, 1]]:
            ...
            if 0 <= x+i and x+i < h and 0 <= y+j and y+j < w and c[x+i][y+j] ==
                    '.' and visited[x+i][y+j] == 0:
                deq.addLast([x+i, y+j])
                visited[x+i][y+j] = visited[x][y] + 1
                result = max(result, visited[x+i][y+j])
        # echo deq
var ans = bfs()
