...
struct Solver {
    ...
    void solve() {
        ...
        // vector<pair<int, int>> tmp_whi(N);
        // rep(i, N) { tmp_whi[i] = {max(wh[i].first, wh[i].second), i}; }
        // sort(all(tmp_whi));
        // reverse(all(tmp_whi));
        rep(i, T - 50) {
            int index = i % N;
            auto tmp = output.query({Action_(index, 0, "U", -1)});
            tmp_wh_vec[index % N].emplace_back(tmp);
            queryes.push_back({Action_(index % N, 0, "U", -1)});
