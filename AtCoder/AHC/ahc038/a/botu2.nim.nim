...
# 回転のhash
var zobrist_hash_rotate: seq[seq[uint64]]
...
proc make_zobrist_hash(input: Input) =
    ...
    zobrist_hash_rotate = newSeqWith(2, newSeq[uint64](4))
    for i in 0..<n:
        for j in 0..<n:
    ...
    for i in 0..<2:
        for j in 0..<4:
            zobrist_hash_rotate[i][j] = rand(uint64)
...
#         if i & 1 == 0:
#             result.add((0, (i+2) div 2))
#         else:
#             result.add((0, -((i+1) div 2)))
#     output.V = input.V
#     for i in 0..<input.V-1:
#         output.pl.add((0, (i+2) div 2))
#     var tmp = ".".repeat(input.V * 2)
#     for i in 1..input.V-1:
#         if i mod 2 == 0:
#             tmp[i] = 'R'
#     output.actions.add(tmp)
#     output.actions.add(tmp)
#     return result
...
#     output.V = input.V
#     for i in 0..<input.V-1:
#         output.pl.add((0, (i+1)*2))
#     return result

proc make_tree(input: Input, output: var Output): seq[(int, int)] =
    result = newSeqOfCap[(int, int)](input.V-1)
    if input.V-1 < input.N * 3 div 5:
        var tmp = input.N * 3 / 5 / (input.V-1)
        for i in 0..<input.V-1:
            result.add((0, int((i+1)*tmp)))

        output.V = input.V
        for i in 0..<input.V-1:
            output.pl.add((0, int((i+1)*tmp)))

        # var action = '.'.repeat(input.V*2)
        # for i in 0..<input.V-1:
        #     if i % 4 == 1:
        #         action[i+1] = 'R'
        #     elif i % 4 == 2:
...
# コンストラクタ 初期状態の作成
proc new_state(input: Input, leaves_coord: seq[(int, int)], root: (int, int)): State =
    var
        N = input.N
        M = input.M
        V = input.V
    ...
    return State(N: N, M: M, V: V, V_div_2: V_div_2, s: s, t: t, rotate: rotate, root: root, leaves_state: leaves_state, leaves_coord: leaves_coord)
...
proc make_initial_node(self: State): (Evaluator, Hash) =
    ...
    # hash ^= zobrist_hash_rotate[0][0]
    # hash ^= zobrist_hash_rotate[1][0]
    for i in 0..<self.N:
        for j in 0..<self.N:
            if self.s[i][j] == '1':
...
proc expand(self: State, evaluator: Evaluator, hash: Hash, parent: int,
    ...
    # n_hash ^= zobrist_hash_rotate[0][self.rotate[0]]
    # n_hash ^= zobrist_hash_rotate[1][self.rotate[1]]
    for move in 0..<5:
        var nn_hash = n_hash
        ...
        for rotate_1 in -1..1:
            for rotate_2 in -1..1:
                ...
                var
                    ...
                    evaluator = evaluator
                    n_rotate = [self.rotate[0] + rotate_1, self.rotate[1] + rotate_2]
                    finished = false
                    nnn_hash = nn_hash
                for i in 0..<2:
                    if n_rotate[i] >= 4:
                        ...
                        n_rotate[i] += 4
                # assert 0 <= n_rotate and n_rotate <= 3
                # nnn_hash ^= zobrist_hash_rotate[0][n_rotate[0]]
                # nnn_hash ^= zobrist_hash_rotate[1][n_rotate[1]]
                for i, (x, y) in self.leaves_coord:
                    var index = i div self.V_div_2
                    # var index = i mod 2
                    var nx, ny: int
                    if n_rotate[index] == 0:
                        nx = x + self.root[0] + MOVES_D[move][0]
                        ny = y + self.root[1] + MOVES_D[move][1]
                    elif n_rotate[index] == 1:
                        nx = y + self.root[0] + MOVES_D[move][0]
                        ny = -x + self.root[1] + MOVES_D[move][1]
                    elif n_rotate[index] == 2:
                        nx = -x + self.root[0] + MOVES_D[move][0]
                        ny = -y + self.root[1] + MOVES_D[move][1]
                    elif n_rotate[index] == 3:
                        nx = -y + self.root[0] + MOVES_D[move][0]
                        ny = x + self.root[1] + MOVES_D[move][1]
...
proc move_forward(self: var State, action: Action) =
    ...
    for i, (x, y) in self.leaves_coord:
        if action.catch[i]:
            ...
            # var index = i mod 2
            var nx, ny: int
            if self.rotate[index] == 0:
                nx = x + self.root[0]
                ny = y + self.root[1]
            elif self.rotate[index] == 1:
                nx = y + self.root[0]
                ny = -x + self.root[1]
            elif self.rotate[index] == 2:
                nx = -x + self.root[0]
                ny = -y + self.root[1]
            elif self.rotate[index] == 3:
                nx = -y + self.root[0]
                ny = x + self.root[1]
            if self.s[nx][ny] == '1':
                self.s[nx][ny] = '0'
...
proc beam_search(config: Config, state: State, time: Time): seq[Action] =
    ...
    for turn in 0..<config.max_turn:
        # 時間等に応じて動的にビーム幅の調整
        var passed_time = time.get_passed_time()
        # if passed_time > 0.5:
        #     selector.change_beam_width(5000)
        # elif passed_time > 1.5:
        #     selector.change_beam_width(3000)
...
proc solve(self: var Solver, output: var Output) =
    ...
    var
        ...
        state = new_state(self.input, leaves_coord, root)
        actions = beam_search(config, state, self.time)
    stderr.write(len(actions))
    for action in actions:
        var init_action = ".".repeat(self.input.V * 2)
        init_action[0] = MOVES_S[action.move]
        for i in 0..<self.input.V-1:
            var index = i div state.V_div_2
            # var index = i mod 2
            if action.rotate[index] == -1:
                init_action[i+1] = 'L'
            elif action.rotate[index] == 1:
                init_action[i+1] = 'R'
        for i in 0..<self.input.V-1:
            if action.catch[i]:
                init_action[self.input.V + 1 + i] = 'P'
