...
relay_xy: seq[(int8, int8)]) =
    while true:
        var
            ...
            t: int8 = -1
            rslt: (seq[string], seq[string])
            rslt_seq = newSeqOfCap[(int, int8, int8, seq[string], seq[string])](25)
        if tx == -1:
            break
        ...
        for gxy in space:
            ...
            for i in int8(0)..<int8(5):
                ...
                if flag1 and now_turn + len(to_txy) > rock[tx][ty] and now_turn + len(to_txy) > rock_idx[contena_b[now_turn + len(to_txy)+1][tx][ty]]:
                    ...
                    if relay_xy[contena_b[now_turn + len(to_txy)+1][tx][ty]][0] != -1:
                        (gx, gy) = relay_xy[contena_b[now_turn + len(to_txy)+1][tx][ty]]
                    (flag2, to_gxy) = get_root_to_gxy(i, tx, ty, gx, gy, now_turn + len(to_txy), crane_b, contena_b)

                    if flag2 and
                    can_put(now_turn+len(to_txy)+len(to_gxy)-1, gx, gy, contena_b):
                        t = contena_b[now_turn + len(to_txy)+1][tx][ty]
                        var score = len(to_txy) + len(to_gxy) + abs(gx - contena_b[now_turn + len(to_txy)+1][tx][ty] div 5)
                        crane_num = i
                        rslt = (to_txy, to_gxy)
                        rslt_seq.add((score, int8(crane_num), t, to_txy, to_gxy))

        if len(rslt_seq) > 0:
            rslt_seq.sort(proc (x, y: (int, int8, int8, seq[string], seq[string])): int = cmp(x[0], y[0]))
            (min_len, crane_num, t, rslt[0], rslt[1]) = rslt_seq[rand(len(rslt_seq) div 3)]
            var
                x = crane[crane_num][now_turn][0]
                y = crane[crane_num][now_turn][1]
                catch = int8(0)
...
proc solve(space: seq[(int8, int8)], relay_xy: seq[(int8, int8)]): (bool, seq[seq[string]], seq[seq[array[5, int8]]]) =
    var
        turn = 0
        ans = first_ans
    ...
    for now_turn in 0..<110:
        ...
        #新規の追加
        move_to_b(now_turn, ans, contena_b, crane_b, crane, target, A_n, bomed, A_idx, rock, rock_idx, out_time, sp, relay_xy)
...
proc climb_hill(): (seq[seq[string]], seq[seq[array[5, int8]]], seq[(int8, int8)]) =
    var
        ...
        relay_xy: seq[(int8, int8)] = newseqwith(25, (int8(-1), int8(-1)))
        # space_not_use: set[int8] = {0, 1, 5, 6, 10, 11, 15, 16, 20, 21, 13, 3, 23, 12, 2, 22}
        # space: set[int8] = {7, 8, 17, 18}
        best_score = 10000
        cnt = 0
        (ok, last_ans, last_contena_b) = solve(trans_space(space), relay_xy)
    if ok:
    ...
    while true:
        ...
        if mode == 0:
            ...
            var
                (ok, ans, contena_b) = solve(trans_space(space), relay_xy)
                score = get_score(ans)
            if ok and score < best_score:
                echo ("1-0", score, best_score)
    ...
    return (last_ans, last_contena_b, trans_space(space))

proc climb_hill_2(ans: var seq[seq[string]], contena_b: seq[seq[array[5, int8]]], space: seq[(int8, int8)]) =
    var
        relay_xy: seq[(int8, int8)] = newseqwith(25, (int8(-1), int8(-1)))
        decided_contena: set[int8]
    ...
    for idx, i in ans:
        ...
        for now_turn, j in i:
            ...
            y += UDLRS_table[j][1]
            if j == "Q" and y != 4:
                decided_contena.incl(contena_b[now_turn+1][x][y])
                relay_xy[contena_b[now_turn+1][x][y]] = (int8(x), int8(y))
    ...
    while true:
        ...
        #交換
        if mode == 0:
            var v, w = sample(decided_contena)
            shuffle(relay_xy)

            var
                (ok, tmp_ans, tmp_contena_b) = solve(space, relay_xy)
                score = get_score(tmp_ans)
            echo ("2-0", ok, score, best_score)
            if ok and score < best_score:

                ans = tmp_ans
                best_score = score
            else:
                swap(relay_xy[v], relay_xy[w])


        #移動
        elif mode == 1:
            var
                v = sample(decided_contena)
                i, j = sample([-1, 0, 1])
                (x, y) = relay_xy[v]
                x_n, y_n: int8
            if 0 <= x+i and x+i < 5:
...
proc main() =
    first_set_up()
    calc_dp()
    var (ans, contena_b, space) = climb_hill()
    climb_hill_2(ans, contena_b, space)
    output(ans)
