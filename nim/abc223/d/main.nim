...
var
    n, m = ii()
    ab = newTable[int, HashSet[int]]()
    ba = newTable[int, HashSet[int]]()
    ans = newSeqOfCap[int](n)
    all_num = (1..n).toSeq().toHashSet()

for i in range(m):
    var a, b = ii()
    # ab.add((a, b))
    if a notin ab:
        ab[a] = initHashSet[int](0)
    if b notin ba:
        ba[b] = initHashSet[int](0)
    ba[b].incl(a)
    ab[a].incl(b)
    all_num.excl(b)

var hq = all_num.toseq().toHeapQueue()
