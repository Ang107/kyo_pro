...
uf = UnionFind(n, vs=[[0] * n, [[] for _ in range(n)]], ops=[lambda a, b: a + b, f])
# print(uf.vs)
yobun = []
for i in range(m):
    a, b = MII()
    ...
    b -= 1
    if uf.same(a, b):
        uf.vs[0][uf.leader(a)] += 1
        uf.vs[1][uf.leader(a)].append(i)
    uf.merge(a, b)
    ab.append((a, b))
    ed[a].append(b)
    ed[b].append(a)
...
for index, i in enumerate(group):
    amari.append(uf.vs[0][uf.leader(i[0])])
    eds.append(deque(uf.vs[1][uf.leader(i[0])]))
...
for i in range(len(amari)):
    for _ in range(k, min(len(amari), k + amari[i])):
        if k == i:
            k += 1
        # print(i, k, (eds[i][0] + 1, ab[eds[i][0]][0] + 1, group[k][0] + 1))
        if k < len(group):
            ans.append((eds[i][0] + 1, ab[eds[i][0]][0] + 1, group[k][0] + 1))
            eds[i].popleft()
            k += 1
