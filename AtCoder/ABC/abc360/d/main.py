...
# 蟻の順番
tmp = {i: j for i, j in enumerate(x)}
tmp = sorted(tmp.items(), key=lambda x: x[1])

bef = [i[0] for i in tmp]
trans = {j: i for i, j in enumerate(bef)}
# print(bef)
tmp = []
for i, j in zip(s, x):
    if i == "0":
...
af = [i[0] for i in tmp]
# print(af)
tmp = [trans[i] for i in af]
# print(InversionNumberByBIT(tmp))
ss = SortedMultiset()
ans = 0
for i in tmp:
    ans += len(ss) - ss.index_right(i)
    ss.add(i)
print(ans)
