        ...
        for (gx, gy) in space:
            for i in int8(0)..<int8(5):
                ...
                if flag1 and now_turn + len(to_txy) > rock[tx][ty] and now_turn + len(to_txy) > rock_idx[contena_b[now_turn + len(to_txy)+1][tx][ty]]:
                    var (flag2, to_gxy) = get_root_to_gxy(i, tx, ty, gx, gy, now_turn + len(to_txy), crane_b, contena_b)
                    if flag2 and
                    len(to_txy) + len(to_gxy) + abs(gx - contena_b[min(max_turn-1, now_turn + len(to_txy)+1)][tx][ty] div 5) < min_len and
                    can_put(now_turn+len(to_txy)+len(to_gxy)-1, gx, gy, contena_b):
                        t = contena_b[now_turn + len(to_txy)+1][tx][ty]
                        min_len = len(to_txy) + len(to_gxy) + abs(gx - contena_b[min(max_turn-1, now_turn + len(to_txy)+1)][tx][ty] div 5)
                        crane_num = i
                        rslt = (to_txy, to_gxy)
...
proc `$`(i: Action): string =
    return $("target: ", i.t, "txy: ", i.txy, "gxy: ", i.gxy, "to_goal: ", i.to_goal, "in_ed: ", i.in_ed)
...
A_n: var seq[seq[int8]], ): int =
    ...
    var max_start_turn = 0
    for (ac, crane_num) in zip(actions, alc):
        max_start_turn = max(max_start_turn, len(ans[crane_num]))
        var
...
seq[seq[int8]]
            ...
            for j in v:
                for k in v:
                    var
                        ...
                        tmp_A_n = A_n
                        score = try_allocation(tmp_ans, tmp_contena_b, tmp_crane_b, tmp_crane, actions, v, tmp_out_time, tmp_A_n)
                    # echo (v, score)
                    if score < best_score:
                        best_score = score
                        last_ans = tmp_ans
    ...
    return (best_score != 1000, last_ans, last_contena_b, last_crane_b, last_crane, last_out_time, last_A_n)
...
proc solve_1(actions: seq[Action]): (bool, seq[seq[string]]) =
    ...
    for now_turn in 0..<100:
        ...
        # for i in 0..<5:
        #     echo last_ans[i].join("")
        #衝突回避
        # var prv_len = get_sum(last_ans)
        collision_avoidance(now_turn, last_ans, last_contena_b, last_crane_b, last_crane, last_bomed)
        # var nxt_len = get_sum(last_ans)

        # deb += (nxt_len - prv_len)
        # for i in last_ans:
        #     echo i.join("")
        # for i in last_contena_b[now_turn+1]:
        #     echo i
        # echo last_A_n
    # echo deb
    return (fin, last_ans)
