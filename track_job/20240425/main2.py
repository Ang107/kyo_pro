...
def read_csv_with_error_handling(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise ValueError("エラー: ファイルが空です。")
    except pd.errors.ParserError:
        raise ValueError("エラー: ファイルの解析に失敗しました。")
    except Exception as e:
        raise Exception(f"予期せぬエラーが発生しました: {str(e)}")


def validate_data_format(df: pd.DataFrame, required_columns: List[str]) -> None:
    """
    指定されたDataFrameのカラムを確認し、必要なカラムが含まれていること、
    および各カラムのデータが指定されたフォーマットに合致していることを検証します。
    """
    missing_columns = [
    ...
    if missing_columns:
        raise ValueError(
            f"エラー: ファイル内に必要なカラム '{', '.join(missing_columns)}' が存在しません。"
        )
    ...
    for column in required_columns:
        if column in df.columns:
            invalid_data = [
                ...
                if not check_format(column, val)
            ]
            if invalid_data:
                raise ValueError(
                    f"エラー: '{column}' 内に不適切なフォーマットのデータが見つかりました。 不適切なデータ: {invalid_data}"
                )
...
def process_entry_and_score_logs(
    ...
    validate_data_format(entry_log, REQUIRED_ENTRY_COLUMNS)
    validate_data_format(score_log, REQUIRED_SCORE_COLUMNS)
