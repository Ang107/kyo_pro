...
def main(lines: list[str]):
    ...
    L = SortedSet([-l * 1000000 + idx for idx, (l, r) in enumerate(LR)])
    R = SortedSet([-r * 1000000 + idx for idx, (l, r) in enumerate(LR)])
    ...
    for i in range(q):
        ...
        if query[0] == 1:
            ...
            x -= 1
            l_old, r_old = LR[x]
            L.discard(-l_old * 1000000 + x)
            L.add(-l_new * 1000000 + x)
            R.discard(-r_old * 1000000 + x)
            R.add(-r_new * 1000000 + x)
            LR[x] = [l_new, r_new]
        else:
            x = query[1]
            result = 0
