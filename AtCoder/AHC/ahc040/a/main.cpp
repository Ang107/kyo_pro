...
struct Output {
    pair<int, int> query(const vector<Put> &puts) {
        int size = puts.size();
        cout << size << el;
        rep(i, size) {
            if (i == size - 1) {
                cout << puts[i].p << ' ' << puts[i].r << ' ' << puts[i].d << ' '
                     << puts[i].b << endl;
            } else {
                cout << puts[i].p << ' ' << puts[i].r << ' ' << puts[i].d << ' '
                     << puts[i].b << el;
...
struct Solver {
    Input input;
    Output output;
    vector<pair<vector<Put>, pair<int, int>>> queryed;
    Solver(const Input &input) : input(input) {}
    void make_neighbor(int max_size, vector<vector<pair<int, int>>> &cands,
                       int sig) {
        // 現在の候補を元に、新たな候補を追加する
        int init_size = cands.size();
        while (cands.size() < max_size) {
            vector<pair<int, int>> cand = cands[xorshift64::next() % init_size];
            rep(i, N) {
                cand[i].first += generate_normal_random(sig);
                cand[i].second += generate_normal_random(sig);
            }
            cands.emplace_back(cand);
        }
    }
    pair<int, int> test(const vector<Put> &puts,
                        const vector<pair<int, int>> &cand) {
        assert(puts.size() == cand.size());
        // 各辺の長さがcandのとき、putsで指定された置き方で置いた場合のW,Hを返す。
        int n = puts.size();
        static vector<pair<int, int>> dl(n);
        static vector<pair<int, int>> dr(n);
        static vector<pair<int, int>> ur(n);
        int w = 0;
        int h = 0;
        rep(i, n) {
            Put put = puts[i];
            ...
            if (put.d == "U") {
                ...
                rep(j, n) {
                    ...
                    if (l < dl[j].second and dl[j].second < r) {
                        chmax(u, dl[j].first);
                    }
                    if (l < dr[j].second and dr[j].second < r) {
                        chmax(u, dr[j].first);
                    }
                }
                d = u + h;
                ...
                rep(j, n) {
                    ...
                    if (u < ur[j].first and ur[j].first < r) {
                        chmax(l, ur[j].second);
                    }
                    if (l < dr[j].first and dr[j].first < r) {
                        chmax(l, dr[j].second);
                    }
                }
                r = l + w;
    ...
    void solve() {
        int max_size = 1000000 / N;
        vector<vector<pair<int, int>>> cands;
        cands.reserve(max_size);
        cands.emplace_back(input.wh);
        make_neighbor(max_size, cands, Sig);
        }
};
