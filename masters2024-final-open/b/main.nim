...
proc out_s(x, y: int): int =
    echo "S ", x, " ", y
    return ii()

proc input(): (int, int, float, float, int, int, seq[(int, int)], seq[(int, int, int, int)]) =
    let
        N, M = ii()
        ep, dlt = si().parseFloat()
        sx, sy = ii()
    var
        P = newseqofcap[(int, int)](10)
        L = newSeqOfCap[(int, int, int, int)](M)
    echo "#", (N, M, ep, dlt, sx, sy)
    for _ in range(N):
        P.add((ii(), ii()))
    for _ in range(M):
        L.add((ii(), ii(), ii(), ii()))
