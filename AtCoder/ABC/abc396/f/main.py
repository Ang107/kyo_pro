...
for i in a:
    tmp.append(st.prod(i + 1, m))
    res += tmp[-1]

    if i + 1 < m:
        one_plus.append(st[i + 1])
    else:
        one_plus.append(0)
    st[i] = st[i] + 1
st2 = SegTree(lambda x, y: x + y, 0, m)
tmp_rev = []
for i in a[::-1]:
    tmp_rev.append(st2.prod(0, i))
    if i - 1 >= 0:
        one_minus.append(st2[i - 1])
    else:
        one_minus.append(0)
    st2[i] = st2[i] + 1
tmp_rev = tmp_rev[::-1]
one_minus = one_minus[::-1]
# for i in range(n):
#     tmp[i] += tmp_rev[i]
...
for i in range(1, m):
    ans[i] = ans[i - 1]
    for j in pos[m - i]:
        print(i, m - i, j, tmp[j], one_minus[j], one_plus[j])
        ans[i] -= tmp[j]
        ans[i] += tmp_rev[j]
        # ans[i] += one_minus[j]
        # ans[i] += one_plus[j]
