...
constexpr float TIME_LIMIT = 4.9;
struct Input {
    // todo
    int N;
    vector<tuple<int, int, int>> xyr;
    void input() {
        cin >> N;
        xyr = vector<tuple<int, int, int>>(N);
        rep(i, N) {
            int x, y, r;
            cin >> x >> y >> r;
            xyr[i] = {x, y, r};
...
struct Rectangle {
    int x, y, r;
    int a, b, c, d;
    Rectangle(int x, int y, int r, int a, int b, int c, int d)
        : x(x), y(y), r(r), a(a), b(b), c(c), d(d) {}
    int get_s() { return (c - a) * (d - b); }
    int get_s(int a, int b, int c, int d) { return (c - a) * (d - b); }
    bool ok() {
        return a <= x and x < c and b <= y and y < d and 0 <= a and a < c and
               c <= 10000 and 0 <= b and b < d and d <= 10000;
    }
    int diff() { return abs(get_s() - r); }
    int diff(int a, int b, int c, int d) { return abs(get_s(a, b, c, d) - r); }
};
struct SA {
    double START_TEMP = 30000;
    double END_TEMP = 1000;
    RNG rng;
    SA() {}
    SA(double START_TEMP, double END_TEMP)
        : START_TEMP(START_TEMP), END_TEMP(END_TEMP) {}
    double temperature(double progress) {
        // progress(0~1)を元に現在の温度を計算して返す。
        return START_TEMP * pow(END_TEMP / START_TEMP, progress);
    }
    bool is_accept(double progress, int before_score, int after_score) {
        // 近傍を受理するかを返す。
        return before_score >= after_score or
               rng.randomDouble() <
                   exp((-after_score + before_score) / temperature(progress));
    }
    double threshold(double progress) {
        // 近傍を受理する閾値を返す。
        return temperature(progress) * log(rng.randomDouble());
    }
};
...
class Solver {
  ...
  public:
    Solver(Input input, Timer timer) : in(input), timer(timer) {}
    void init_state() {
        rectangles.reserve(in.N);
        rep(i, in.N) {
            auto [x, y, r] = in.xyr[i];
            rectangles.push_back(Rectangle(x, y, r, x, y, x + 1, y + 1));
            vertical[y] = i;
            horizon[x] = i;
        }
    }
    bool is_overlap(int idx, const Rectangle &rect) {
        int a = rect.a;
        int b = rect.b;
        int c = rect.c;
        int d = rect.d;
        rep(i, in.N) {
            ...
            int cc = r.c;
            int dd = r.d;
            int overlap_w = min(c, cc) - max(a, aa);
            int overlap_h = min(d, dd) - max(b, bb);
    ...
    void sa() {
        SA sim;
        auto best_rectangles = rectangles;
        int iter = 0;
        double progress;
        vector<int> neighbors = {0, 1};
        int neighbor_size = neighbors.size();
        int current_score_diff = 0;
        int best_score_diff = 0;
        int MAX_WIDTH = 10;
        // 一定ターン経過ごとに，最も割合が悪いものの付近を破壊するとかどうだろうか
        while (1) {
            if (iter % 100 == 0) {
                float now = timer.elapsed();
                debug(iter, current_score_diff);
                if (now > TIME_LIMIT) {
                    break;
                }
                progress = now / TIME_LIMIT;
            }
            int mode = neighbors[rng.random32(neighbor_size)];
            if (mode == 0) {
                int idx = rng.random32(in.N);
                int abcd = rng.random32(4);
                int length = rng.randomRange32(1, MAX_WIDTH);
                int plusminus = rng.random32(2);
                if (plusminus) {
                    length *= -1;
                }
                Rectangle new_rect = rectangles[idx];
                if (abcd == 0) {
                    new_rect.a += length;
                } else if (abcd == 1) {
                    new_rect.b += length;
                ...
                if (!new_rect.ok() or is_overlap(idx, new_rect)) {
                    continue;
                }
                int prev_score = rectangles[idx].diff();
                int new_score = new_rect.diff();
                if (sim.is_accept(progress, prev_score, new_score)) {
                    rectangles[idx] = new_rect;
                    current_score_diff += prev_score - new_score;
                    if (current_score_diff > best_score_diff) {
                        best_score_diff = current_score_diff;
                        best_rectangles = rectangles;
                    }
                }
            } else if (mode == 1) {
                int idx = rng.random32(in.N);
                int ab = rng.random32(2);
                int length = rng.randomRange32(1, MAX_WIDTH);
                int plusminus = rng.random32(2);
                if (plusminus) {
                    length *= -1;
                ...
                int idx = rng.random32(in.N);
                int ab = rng.random32(2);
                int length = rng.randomRange32(1, 3);
        ...
        debug(best_score_diff, iter);
        debug(current_score_diff, iter);
    }
    void solve() {
        init_state();
        sa();
    ...
    void print() {
        for (const auto &rct : rectangles) {
            cout << rct.a << " " << rct.b << " " << rct.c << " " << rct.d << el;
        }
    }
    Input in;
    Timer timer;
