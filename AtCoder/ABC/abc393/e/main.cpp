...
vector<pair<ll, ll>> fact(ll n, vector<ll> &primes) {
...
int main() {
    auto primes = get_primes(1000000 + 1);

    int n, k;
    cin >> n >> k;
    vector<int> a(n);
    int max_ = 0;
    vector<int> dd(1000000 + 1, 0);
    vector<int> cnt(1000000 + 1, 0);
    vector<int> ans(1000000 + 1, 1);
    rep(i, n) {
        cin >> a[i];
        chmax(max_, a[i]);
        dd[a[i]] += 1;
    ...
    rep(i, max_ + 1) {
        int res = 1;
        if (dd[i] != 0) {
            auto f = fact(i, primes);
            vector<int> cand;
            cand.push_back(1);
            for (auto [j, l] : f) {
                vector<int> ncand;
                for (auto m : cand) {
                    rep(p, l + 1) { ncand.push_back(m * ipow(j, (ll)p)); }
                }
                swap(cand, ncand);
            }
            for (auto j : cand) {
