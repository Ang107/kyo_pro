...
type
    ...
    # コンストラクタ
proc new_action(move: int, rotate: array[3, int], catch: uint64): Action {.inline.} =
...
proc make_tree(input: Input, output: var Output): seq[(int, int)] =
    if MODE == 3:
        ...
        output.pl.add((0, input.N div 4))
        output.pl.add((1, input.N div 4))

        var st = input.N div 2
        if input.V-2 < input.N div 2:
            var tmp = input.N / 2 / (input.V-1)
            for i in 0..<input.V-3:
                result.add((0, int((i+1)*tmp)))

            output.V = input.V
            for i in 0..<input.V-3:
                output.pl.add((2, int((i+1)*tmp)))
            return result
        else:
            for i in 0..<input.V-3:
                result.add((0, i+1))
...
# コンストラクタ 初期状態の作成
proc new_state(input: Input, leaves_coord: seq[(int, int)], root, edge_1, edge_2: (int, int)): State =
    var
        N = input.N
        M = input.M
        V = input.V
    ...
    return State(N: N, M: M, V: V, s: s, t: t, rotate: rotate, root: root, edge_1: edge_1, edge_2: edge_2, leaves_state: leaves_state, leaves_coord: leaves_coord)
    ...
    if MODE == 3:
        ...
        for move in 0..<5:
            ...
            for rotate_1 in -1..1:
                var
                    ex, ey: int
                    n_rotate = self.rotate[0] + rotate_1
                if n_rotate >= 4:
                    n_rotate -= 4
                elif n_rotate < 0:
                    n_rotate += 4
                ...
                if n_rotate == 0:
                    ex = self.edge_1[0] + self.root[0] + MOVES_D[move][0]
                    ey = self.edge_1[1] + self.root[1] + MOVES_D[move][1]
                elif n_rotate == 1:
                    ex = self.edge_1[1] + self.root[0] + MOVES_D[move][0]
                    ey = -self.edge_1[0] + self.root[1] + MOVES_D[move][1]
                elif n_rotate == 2:
                    ex = -self.edge_1[0] + self.root[0] + MOVES_D[move][0]
                    ey = -self.edge_1[1] + self.root[1] + MOVES_D[move][1]
                elif n_rotate == 3:
                ...
                for rotate_2 in -1..1:
                    var
                        eex, eey: int
                        n_rotate = self.rotate[1] + rotate_1 + rotate_2
                    if n_rotate >= 4:
                        n_rotate -= 4
                    elif n_rotate < 0:
                        n_rotate += 4
                    ...
                    if n_rotate == 0:
                        eex = self.edge_2[0] + ex
                        eey = self.edge_2[1] + ey
                    elif n_rotate == 1:
                        eex = self.edge_2[1] + ex
                        eey = -self.edge_2[0] + ey
                    elif n_rotate == 2:
                        eex = -self.edge_2[0] + ex
                        eey = -self.edge_2[1] + ey
                    elif n_rotate == 3:
                    ...
                    for rotate_3 in -1..1:
                        if rotate_1 == 0 and rotate_2 == 0 and rotate_3 == 0 and move == 0:
                            continue
                        var
                            action = new_action(move, [rotate_1, rotate_2, rotate_3], 0)
                            evaluator = evaluator
                            n_rotate = self.rotate[2]+rotate_1+rotate_2+rotate_3
                            finished = false
                            nnn_hash = nn_hash
                        if n_rotate >= 4:
                            n_rotate -= 4
            ...
            for rotate_1 in -1..1:
                ...
                for rotate_2 in -1..1:
                    if rotate_1 == 0 and rotate_2 == 0 and move == 0:
                        continue
                    var
                        action = new_action(move, [rotate_1, rotate_2, 0], 0)
                        evaluator = evaluator
                        nn_rotate = n_rotate
                        finished = false
                        nnn_hash = nn_hash
...
proc move_forward(self: var State, action: Action) =
    if MODE == 3:
        ...
        var ex, ey: int
        if self.rotate[0] == 0:
            ex = self.edge_1[0] + self.root[0]
            ey = self.edge_1[1] + self.root[1]
        elif self.rotate[0] == 1:
            ex = self.edge_1[1] + self.root[0]
            ey = -self.edge_1[0] + self.root[1]
        ...
        for i, (x, y) in self.leaves_coord:
            if action.catch[i]:
                ...
                if self.rotate[2] == 0:
                    nx = x + ex
                    ny = y + ey
                elif self.rotate[2] == 1:
                    nx = y + ex
                    ny = -x + ey
                elif self.rotate[2] == 2:
                    nx = -x + ex
                    ny = -y + ey
                elif self.rotate[2] == 3:
                    nx = -y + ex
                    ny = x + ey
                if self.s[nx][ny] == '1':
                    self.s[nx][ny] = '0'
...
proc solve(self: var Solver, output: var Output) =
    ...
    var
        ...
        state = new_state(self.input, leaves_coord, root, (0, output.pl[0][1]), (0, output.pl[1][1]))
        actions = beam_search(config, state, self.time)
    stderr.writeLine(actions.len())
    if MODE == 3:
        for action in actions:
            ...
            for i in 1..<self.input.V:
                if i == 1:
                    ...
                    if action.rotate[2] == -1:
                        init_action[i] = 'L'
                    elif action.rotate[2] == 1:
                        init_action[i] = 'R'
            for i in 0..<self.input.V-1:
                if action.catch[i]:
                    init_action[self.input.V + 3 + i] = 'P'
...
proc main() =
    ...
    if time.get_passed_time() > 2.5:
        stderr.writeLine("!!! TLE DANGER !!! ", time.get_passed_time())
