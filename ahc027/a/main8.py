...
def calc_around_avr(x,y):
    ...
    for i in range(-n//6,n//6 + 1):
        for j in range(-n//6,n//6 + 1):
            if 0 <= x+i < n and 0 <= y+j < n:
                temp.append(dirt[x+i][y+j]/ (0.9 ** (abs(i) + abs(j))))
    return sum(temp)/len(temp)

def calc_around_avr2(x,y):
    temp = []
    for i in range(-n//4,n//4 + 1):
        for j in range(-n//4,n//4 + 1):
    ...
    while count < 10 ** 5 - 35000:
        ...
        for i,j in enumerate(Vec):
            if j and 0 <= x+around4[i][0] < n and 0 <= y+around4[i][1] < n:
                ...
                l.append(dirt[x+around4[i][0]][y+around4[i][1]]**coe1 *calc_gain(x+around4[i][0],y+around4[i][1])**coe2 *calc_around_avr2(x+around4[i][0],y+around4[i][1])**coe3 * max(0.3,1 -  200*  visited_mid[x+around4[i][0]][y+around4[i][1]] /  count)**coe4 )
