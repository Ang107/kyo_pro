...
proc new_config(max_turn, beam_width, tour_capacity, hashmap_capacity: int): Config =
...
type
    MaxSegTree = SegTreeType[(Cost, int)](op, () => (-2147483648, -1))
    Selector = object
        beam_width: int
...
# 速度にかなり寄与する部分なので大事
proc expand(self: State, evaluator: Evaluator, hash: Hash, parent: int, selector: var Selector){.inline.} =
    for d in 0..<4:
        var
            action: Action
