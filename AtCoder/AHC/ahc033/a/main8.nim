...
var
    ...
    best_move_1 = newSeqWith(3125, newSeqOfCap[int](4))
    first_b: set[int8]
...
proc get_can_del(b: set[int8]): set[int8] =
    for i in 0..<5:
        for j in 0..<5:
            if int8(i*5+j) in b:
                result.incl(int8(i*5+j))
...
proc f(v: int): int =
    ...
    # echo (v)
    var
        tmp: array[5, int8] = [-1, -1, -1, -1, -1]
        v_n = v
    for i in [4, 3, 2, 1, 0]:
        tmp[i] = int8(v_n div (5**i))
        v_n = v_n mod (5**i)
    ...
    for i in 0..<5:
        if tmp[i] != 0:
            yama.incl(A[i][^(tmp[i]+1)])


        for j in A[i][1 ..< ^tmp[i]]:
            b.incl(j)

    var can_del = get_can_del(b)
    # echo b
    # echo can_del
    # echo yama

    if len(can_del * yama) == 0:
        memo[v] = len(b) - len(can_del)
        # echo ("a", len(b) - len(can_del))
        return len(b) - len(can_del)
    else:
        for i in 0..<5:
            if tmp[i] != 0 and A[i][^(tmp[i]+1)] in can_del:
                rslt = min(rslt, f(v - 5**i))
        memo[v] = rslt
        # echo ("b", rslt)
        return rslt

proc calc_best_move_from_memo() =
    for i in 1..<3125:
        var min_cost = 100
        for j in 0..<5:
            var tmp = (i mod (5 ** (j+1))) div (5 ** j)
            if tmp > 0:
                min_cost = min(min_cost, memo[i - 5**j])
        memo[i] = max(memo[i], min_cost)
    ...
    for i in 1..<3125:
        ...
        for j in 0..<5:
            var tmp = (i mod (5 ** (j+1))) div (5 ** j)
            if tmp > 0:
                # echo (i, j, memo[i], memo[i - 5**j])
                if min_cost > memo[i - 5**j]:
                    min_cost = memo[i - 5**j]
                    best_move_1[i] = @[j]
                elif min_cost == memo[i - 5**j]:
                    best_move_1[i].add(j)
    # var
    #     way = newSeqOfCap[int](30)
    #     v = 3124
    ...
    #     now.incl(A[i][0])
    # while v != 0:
    #     # echo (v, best_move_1[v])
    #     # echo memo[v]
    #     # echo ("ADD", A[best_move_1[v][0]][idx[best_move_1[v][0]]])
    #     now.incl(A[best_move_1[v][0]][idx[best_move_1[v][0]]])
    #     # echo get_now_b(now)

    #     way.add(best_move_1[v][0])
    #     idx[best_move_1[v][0]] += 1
    #     v -= 5 ** best_move_1[v][0]
...
proc get_best_move(A_n: seq[seq[int8]]): int8 =
    ...
    for i in 0..<5:
        tmp += (len(A_n[i])) * (5 ** i)
    if len(best_move_1[tmp]) > 0:
        return int8(sample(best_move_1[tmp]))
    else:
        return -1
...
proc solve(space: seq[int8]): (bool, seq[seq[string]]) =
    var
        turn = 0
        ans = first_ans
...
proc climb_hill(): seq[seq[string]] =
    var
        ...
        # space_not_use: seq[int8] = @[0, 1, 5, 6, 7, 8, 10, 11, 15, 16, 17, 18, 20, 21]
        space: seq[int8] = @[13, 3, 23, 12, 2, 22]
        space_set: set[int8] = {13, 3, 23, 12, 2, 22}

        # space_not_use: set[int8] = {0, 1, 5, 6, 10, 11, 15, 16, 20, 21, 13, 3, 23, 12, 2, 22}
        # space: set[int8] = {7, 8, 17, 18}
        best_score = 10000
        cnt = 0
        (ok, last_ans) = solve(space)
    if ok:
    ...
    while true:
        if cputime() - START_TIME > 2.9:
            break
        var mode = sample([0, 0, 0, 1, 1, 1, 2, 2, 3, 4])
        #削除
        if mode == 0:
            var
                v = rand(len(space)-1)
                num = space[v]
            if num notin space_set:
                continue
            # echo ("0", space)
            space.delete(v)
            # echo ("0", space)
