                        ...
                        for (int jj = j; jj < N; jj++) {
                            ...
                            Evaluator n_evaluator =
                                Evaluator(evaluator.score_a_day + add_score,
                                          ...
                                          evaluator.now_K - 2 * STATION_COST -
                                              LINE_COST * dis);
                            int idx1 = (i << 8) | j;
                            int idx2 = (ii << 8) | jj;
                            Action n_action =
                            ...
                            selector.push(Candidate(n_action, n_evaluator,
                                                    n_hash, parent),
                                          false);
                        }
                    }
...
beam_search(Config &config, const State &state,
    ...
    config.beam_width = min((size_t)15000, beam_width * 30);
    ...
    rep(i, T) {
        ...
        ++turn;
        if (turn == 4) {
            selector.change_beamwidth(min((size_t)15000, beam_width * 10));
        }
        if (turn == 10) {
            selector.change_beamwidth(min((size_t)15000, beam_width));
        }
    ...
    for (const auto &[cost, actions] : solutions) {
        ...
        rep(i, actions.size()) {
            int ii = actions[i].idx1 >> 8;
            int jj = actions[i].idx1 & 255;
            tmp.emplace_back(ii, jj);
            if (actions[i].idx2 != -1) {
                int ii = actions[i].idx2 >> 8;
                int jj = actions[i].idx2 & 255;
                tmp.emplace_back(ii, jj);
            }
        }
        res.push_back(tmp);
...
void add_task(int index, const vector<pair<int, int>> &stations,
    ...
    if (route.empty()) {
        ...
        // assert(mp[route.front().first][route.front().second] != -1);
        if (mp[route.front().first][route.front().second] == 0) {
            cerr << "use station" << el;
            auto [ii, jj] = route.front();
            int add_score = add_station(route.front().first,
                                        route.front().second, people, visited);
            add_scores.push_back(add_score);
            gain.push_back(-STATION_COST);
            mp[ii][jj] = 1;
            wt[ii][jj] = 0;
        ...
        // assert(route.back() == make_pair(i, j));
        int add_score = add_station(i, j, people, visited);
        add_scores.push_back(add_score);
...
void add_task(int index, int now_turn, const vector<pair<int, int>> &stations,
    ...
    if (is_last) {
        ...
        for (auto &[dx, dy] : dxy) {
            ...
            if (ok and mp[i][j] <= mp[nx][ny]) {
                ...
                // if (mp[nx][ny] == 0) {
                //     new_sub_score += 1000000000;
                // }
                if (new_sub_score > max_sub_score) {
                    wt[i][j] = 0;
                    // cerr << "! " << new_sub_score << " " << max_sub_score <<
                    // el;
    ...
    if (route.empty()) {
        ...
        if (mp[route.front().first][route.front().second] == 0) {
            ...
            // auto [ii, jj] = route.front();
            // auto [add_score, sub_score] =
            //     calc_add_score(route.front().first, route.front().second,
            //                    now_turn, people, visited, first_visit);
            // add_scores.push_back(add_score);
            // gain.push_back(-STATION_COST);
            // mp[ii][jj] = 1;
            // wt[ii][jj] = 0;
...
int main() {
    ...
    // while (need_ijs.size() < 00 and used_cnt < 2500) {
    //     int x = xorshift() % N;
    //     int y = xorshift() % N;
    //     if (used[x][y]) {
    //         continue;
