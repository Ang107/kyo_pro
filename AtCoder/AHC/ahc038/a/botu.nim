...
# 回転のhash
var zobrist_hash_rotate: seq[uint64]
...
proc make_zobrist_hash(input: Input) =
    ...
    zobrist_hash_rotate = newSeq[uint64](4)
    for i in 0..<n:
        for j in 0..<n:
            zobrist_hash_s[i][j] = rand(uint64)
            zobrist_hash_coord[i][j] = rand(uint64)
    for i in 0..<4:
        zobrist_hash_rotate[i] = rand(uint64)
...
type
    ...
    # コンストラクタ
proc new_action(move: int, rotate: int, catch: uint64): Action {.inline.} =
...
#             r = -1
#         for i in 0..<input.V-1:
#             if i+1 < input.N * 3 div 5:
#                 result.add((0, i+1))
#             else:
#                 if i < input.N * 3 div 5:
#                     r = i
#                 else:
#                     result.add((i-r, r))
...
#                     output.pl.add((0, i))
#                 else:
#                     output.pl.add((r+1, i-r))
#         var action = '.'.repeat(input.V*2)
#         for i in r+2..<input.V:
#             action[i] = 'R'
...
proc new_state(input: Input, leaves_coord: seq[(int, int)], root: (int, int)): State =
    ...
    return State(N: N, M: M, V: V, s: s, t: t, rotate: rotate, root: root, leaves_state: leaves_state, leaves_coord: leaves_coord)
...
proc expand(self: State, evaluator: Evaluator, hash: Hash, parent: int,
    ...
    # n_hash ^= zobrist_hash_rotate[self.rotate]
    for move in 0..<5:
        var nn_hash = n_hash
        if 0 <= self.root[0] + MOVES_D[move][0] and
            ...
            continue
        for rotate in -1..1:
            if rotate == 0 and move == 0:
                continue
            var
                action = new_action(move, rotate, 0)
                evaluator = evaluator
                n_rotate = self.rotate + rotate
                finished = false
                nnn_hash = nn_hash
            ...
            # nnn_hash ^= zobrist_hash_rotate[n_rotate]
            for i, (x, y) in self.leaves_coord:
                ...
                if 0 <= nx and nx < self.N and 0 <= ny and ny < self.N:
                    if self.leaves_state[i] and self.s[nx][ny] == '0' and self.t[nx][ny] == '1':
                        evaluator.put += 1
                        evaluator.caught -= 1
                        action.catch.add(i)
                    ...
                    elif not self.leaves_state[i] and self.s[nx][ny] == '1' and self.t[nx][ny] == '0':
                        evaluator.caught += 1
                        action.catch.add(i)
                        # nnn_hash ^= zobrist_hash_s[nx][ny]
