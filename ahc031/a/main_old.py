...
def yamanobori2(ans, tate_haiti, over, h, S, A):
    cnt = 0
    len_ans = len(ans)
    len_h = len(h)
    more_good_0 = 0
    ...
    while True:
        ...
        mode = random.choice([0, 0, 0, 1, 2, 3, 4, 4])
        ...
        if mode == 5:
            ...
            tmp1 = set(tate_haiti[idx1][idx2])
            tmp2 = set(tate_haiti[idx1][idx3])
            ...
            if 0 <= idx1 - 1 < len_ans:
                if (
                    len(tmp1 & set(tate_haiti[idx1 - 1][idx2])) != 2
                    or len(tmp2 & set(tate_haiti[idx1 - 1][idx3])) != 2
                ):
                    continue
            ...
            if all([i >= j for i, j in zip(A_minus[idx1], s)]):
                tate_haiti[idx1][idx2], tate_haiti[idx1][idx3] = (
                    tate_haiti[idx1][idx3],
                    tate_haiti[idx1][idx2],
                )
        ...
        # 行の異なる一致点への移動
        if mode == 4:
            if len(tate_haiti[idx1][idx2]) == 2:
                continue
            idx3 = random.choice(range(1, len(tate_haiti[idx1][idx2]) - 1))
            ...
            if idx1 - 1 in range(len(ans)):
                if m in tate_haiti[idx1 - 1][idx2]:
                    flag1 = True
            if idx1 + 1 in range(len(ans)):
                if m in tate_haiti[idx1 + 1][idx2]:
                    flag2 = True
            ...
            other = tate_haiti[idx1 - rev][idx4]

            s = S[idx1]

            for i in other:
                if 1 <= i < 1000 and i not in tate_haiti[idx1][idx4]:
                    prv_l_1 = h[idx2] * (m - l)
                    prv_r_1 = h[idx2] * (r - m)
                    nxt_lr_1 = h[idx2] * (r - l)

                    l_2 = tate_haiti[idx1][idx4].le(i)
                    r_2 = tate_haiti[idx1][idx4].ge(i)
            ...
            if 0 <= idx1 - rev < len_ans:
                if m in tate_haiti[idx1 - rev][idx2]:
                    score_diff -= 1
            ...
            if 0 <= idx1 - rev < len_ans:
                if m in tate_haiti[idx1 - rev][idx2]:
                    score_diff -= 1
                if l + r - m in tate_haiti[idx1 - rev][idx2]:
                    score_diff += 1
            if 0 <= idx1 + rev < len_ans:
                if m in tate_haiti[idx1 + rev][idx2]:
                    score_diff -= 1
            ...
            tmp_l = tmp.lt(num)
            tmp_r = tmp.gt(num)
            other_l = tate_haiti[idx1 - rev][idx2].le(num)
            other_r = tate_haiti[idx1 - rev][idx2].ge(num)
            ...
            if 0 <= idx1 + rev < len_ans:
                if num in tate_haiti[idx1 + rev][idx2]:
                    score_diff_l -= 1
                if other_l in tate_haiti[idx1 + rev][idx2]:
                    score_diff_l += 1
            ...
            if 0 <= idx1 + rev < len_ans:
                if num in tate_haiti[idx1 + rev][idx2]:
                    score_diff_r -= 1
                if other_r in tate_haiti[idx1 + rev][idx2]:
                    score_diff_r += 1
            ...
            if idx1 - 2 * rev in range(len_ans):
                if other_l in tate_haiti[idx1 - 2 * rev][idx2]:
                    flag_l = False
                    continue

            if idx1 - 2 * rev in range(len_ans):
                if other_r in tate_haiti[idx1 - 2 * rev][idx2]:
                    flag_r = False
                    continue
