...
def evaluate(stations):
    ...
    now_turn += 2
    score_a_day += add_score
    gain = [now_K, (T - now_turn) * add_score - 5500]
    for i, j in stations[2:]:
        if now_turn + max(max(0, 6000 - now_K) // score_a_day, 10) > T:
            break
        add_score = add_station(i, j, people, visited)

        now_turn += max(max(0, 6000 - now_K) // score_a_day, 10)

        score_a_day += add_score
        gain.append((T - now_turn) * add_score - 6000)
