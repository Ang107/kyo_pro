...
uf = UnionFind(
    ...
    [lambda x, y: min(x, y), lambda x, y: max(x, y)],
)
...
for _ in range(q):
    ...
    if tmp[0] == 1:
        ...
        x -= 1
        c -= 1
        width = uf.op(x, 1) - uf.op(x, 0) + 1
        prev_c = color[uf.leader(x)]
        ans[prev_c] -= width
        ans[c] += width
        if uf.op(x, 0) > 0 and c == color[uf.leader(uf.op(x, 0) - 1)]:
            uf.merge(x, uf.op(x, 0) - 1)
        if uf.op(x, 1) < n - 1 and c == color[uf.leader(uf.op(x, 1) + 1)]:
            uf.merge(x, uf.op(x, 1) + 1)
        color[uf.leader(x)] = c
    else:
        c = tmp[1]
        c -= 1
