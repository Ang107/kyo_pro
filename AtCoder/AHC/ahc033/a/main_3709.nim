...
proc sorted_target(now_turn: int, target: seq[int8], contena_b: seq[seq[array[5, int8]]]): seq[int8] =
    ...
    # var
    #     tmp1 = newSeqOfCap[(int8, int)](len(target))
    #     tmp2 = newSeqWith(25, -1)
    # for i in 0..<5:
    #     for j in 0..<5:
    #         if contena_b[now_turn + 10][i][j] != -1:
    #             tmp2[contena_b[now_turn + 10][i][j]] = j
    # for i in target:
    #     tmp1.add((i, tmp2[i]))
    # tmp1.sort(proc (x, y: (int8, int)): int = cmp(-x[1], -y[1]))
    # for (p, i) in tmp1:
    #     result.add(p)
    # return result
...
proc solve_1(actions: seq[Action]): (bool, seq[seq[string]]) =
    ...
    for now_turn in 0..<100:
        ...
        var
            part_of_actions = actions[idx..<min(len(actions), idx+len(v))]
            changed = false
        idx += len(part_of_actions)
        ...
        for i in 0..<len(v):
            ...
            var (ok, ans, contena_b, crane_b, crane, out_time, A_n) =
                allocation(last_ans, last_contena_b, last_crane_b, last_crane, last_out_time, last_A_n, part_of_actions[0 ..< ^i], v)
            # echo v
            # for i in part_of_actions[0 ..< ^i]:
            #     echo i
            # echo ok
            if ok:
                changed = true
