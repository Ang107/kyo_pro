def create_solution(
    ...
    # TODO: Implement this function
        ...
        sabun = [None] * (10**5)
        sabun_idx = 0
        sabun_visited = defaultdict(int)

        qry_answered = defaultdict(int)
        ...
        for i, j, k in zip(q1, q2, q3):
            ...
            if k != -1:
                ...
                pokemon_sum[area] += k
                # wariai = 100 * pokemon_num_in_area[pokemon][area] // pokemon_sum[area]
                ...
                #         )
                #         SL[p].discard(wariai)
                #         wariai = 100 * pokemon_num_in_area[p][area] // pokemon_sum[area]
                #         SL[p].add(wariai)
                sabun[sabun_idx] = [area, pokemon, k]
                sabun_idx += 1
            elif k == -1:
                pokemon = i - 1
                if j == 0:
                    ...
                    p_cahnge = defaultdict(int)
                    for a, p, num in sabun[sabun_visited[pokemon] : sabun_idx]:
                        if a in pokemon_num_in_area[pokemon]:
                            change[a] += num
                            if p == pokemon:
                                p_cahnge[a] += num

                    for a, p, num in change.items():
                        prv = (
                            100
                            * (pokemon_num_in_area[pokemon][a] - p_cahnge[a])
                            // (pokemon_sum[a] - num)
                        )
                        af = 100 * pokemon_num_in_area[pokemon][a] // pokemon_sum[a]

                        if prv < j and j <= af:
                            qry_answered[pokemon] += 1
                        elif prv > j and j > af:
                            qry_answered[pokemon] -= 1
                    ...
                    #         cnt += 1
                    result.append(qry_answered[pokemon])
