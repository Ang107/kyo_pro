...
proc clear(self: var Hashmap) =
    for i in 0..<self.s.len():
        self.s[i].setLen(0)
proc new_hashmap(LA: int, split_num: int): HashMap =
    var s = newSeqwith(split_num, newSeqOfCap[(int, Node1)](0))

    return HashMap(s: s, split_num: split_num)

# 見つかったらインデックスを、無ければマイナス1を返す。
proc index(self: HashMap, key: int): int {.inline.} =
    for i, (k, _) in self.s[(key+1) & mask]:
        if k == key:
            return i
    return -1
proc `[]`(self: HashMap, key: int): (int, Node1) =
    return self.s[(key+1) & mask][self.index(key)]
proc `[]=`(self: var HashMap, kv: (int, Node1)) =
    var p = self.index(kv[0])
    if p == -1:
        self.s[(kv[0]+1) & mask].add(kv)
    else:
        self.s[(kv[0]+1) & mask][p][1] = kv[1]
proc update(self: var HashMap, p: int, k: int, v: Node1) =
    if p == -1:
        self.s[(k+1) & mask].add((k, v))
    else:
        self.s[(k+1) & mask][p] = (k, v)
proc get(self: var HashMap, p: int, k: int): (int, Node1) =
    return self.s[(k+1) & mask][p]
proc add(self: var HashMap, k: int, v: Node1) =
    self.s[(k+1) & mask].add((k, v))



iterator ite(self: HashMap): (int, Node1) =
    for i in self.s:
        for j in i:
            yield j
...
proc normal_dijkstra(input: Input, output: var Output, index_list: array[N, seq[
    ...
    while deq.len() > 0:
        ...
        for next in input.G[node2.v_index]:
            ...
            (node2.signal_state == -1 and next in last_colorchange_signal):
                var p = visited[next].index(node2.signal_state)
                if p == -1 or
                    node2.times < visited[next].get(p, node2.signal_state)[1].times:
                    deq.addFirst(new_node2(node2.times, next,
                            node2.signal_state))
                    visited[next].update(p, node2.signal_state,
                                        new_node1(next, node2.times, -1, (node2.v_index, node2.signal_state)))
            else:
                for i in index_list[next]:
                    ...
                    for j in 0..<2:
                        ...
                        if j == 1:
                            ...
                            index = r
                        var p = visited[next].index(index)
                        if p == -1 or
                            node2.times+1 < visited[next].get(p, index)[1].times:
                            deq.addLast(new_node2(node2.times+1, next, index))
                            visited[next].update(p, index, new_node1(next, node2.times+1, index,
    ...
    if next_g != -1:
        ...
        for (signal_state, node1) in visited[g].ite:
        ...
        for i in 0..<nodes.len():
            while nodes[i].change_signal == -1 and nodes[i].frm[0] != -1:
                nodes[i] = visited[nodes[i].frm[0]][nodes[i].frm[1]][1]
        ...
        if not flag:
            ...
            now = visited[frm][signal_state][1]
    ...
    while now.frm[0] != -1:
        ...
        if now.change_signal != -1:
            ...
            actions.add(("s", min(input.LB, input.LA-now.change_signal),
                    now.change_signal, 0))
        now = visited[now.frm[0]][now.frm[1]][1]
    actions.reverse()
    for i in actions:
...
proc beamsearch(input: Input, output: var Output, time: Time) =
    ...
    for turn in 0..<N:
        ...
        for s, cand in cands_array[turn]:
            # 最後の状態を全探索
            for (k, v) in change_times[s][g].ite:
                #遡る
                var now = v
                while now.change_signal == -1 and now.frm[0] != -1:
                    now = change_times[s][now.frm[0]][now.frm[1]][1]
    ...
    for i in 0..<N:
        ...
        #t1 => t2の行動
        if t2 != -1:
            var now = change_times[t1][t2][action.signal][1]
            ...
            while now.frm[0] != -1:
                ...
                now = change_times[t1][now.frm[0]][now.frm[1]][1]
            tmp.reverse()
            for i in tmp:
                output.actions.add(i)
...
proc fast_dijkstra(input: Input, output: var Output, index_list: array[N, seq[
    ...
    var
        ...
        best_score = INF64
    for s in cand:
        for (signal_state, node1) in change_times[s][g].ite:
    ...
    var
        ...
        now = change_times[best_action[0]][g][best_action[1]][1]
    ...
    while now.frm[0] != -1:
        ...
        now = change_times[best_action[0]][now.frm[0]][now.frm[1]][1]
...
proc make_change_times(input: Input, output: Output,
    ...
    proc dijkstra(input: Input, output: Output,
        ...
        while deq.len() > 0:
            var node2 = deq.popFirst()
            var tmp =
                visited[node2.v_index][node2.signal_state][1]
            if node2.times > tmp.times:
                continue
            ...
            for next in input.G[node2.v_index]:
                if node2.signal_state == -1 or not in_a_index[node2.signal_state][next]:
                    ...
                    var p = visited[next].index(node2.signal_state)
                    if p == -1 or node2.times < visited[next].get(p, node2.signal_state)[1].times:
                        deq.addFirst(new_node2(node2.times, next,
                                node2.signal_state))
                        visited[next].update(p, node2.signal_state,
                                            new_node1(next, node2.times, -1, (node2.v_index, node2.signal_state)))
