...
struct Action {
    ...
    double amount;
    int block_cnt;
    Action() : tube_index(0), add_tube(0), amount(0), block_cnt(0) {}
    Action(int tube_index, int add_tube, double amount, int block_cnt)
...
struct Palette {
    ...
    Action discretization(double am, int mode = 1) {
        ...
        if (mode == -1) {
            res.add_tube = ceil(max(0.0, am - amount));
            res.block_cnt =
                floor(am / ((amount + res.add_tube) / (double)block_cnt));
            res.amount =
                ((amount + res.add_tube) / (double)block_cnt) * res.block_cnt;

        } else {
            res.add_tube = ceil(max(0.0, am - amount));
            res.block_cnt =
                ceil(am / ((amount + res.add_tube) / (double)block_cnt));
            res.amount =
                ((amount + res.add_tube) / (double)block_cnt) * res.block_cnt;
        }
        return res;
...
struct Solver {
    ...
    bool normalization(vector<double> &coefficients, int mode = 1) {
        double sum_ = 0;
        for (auto a : coefficients)
            sum_ += a;
        if (sum_ == 0.0)
            return false;
        if (mode == 1 or sum_ < 1.0) {
            for (auto &a : coefficients)
                a /= sum_;
        }
        return true;
    ...
    clime(Color target, const vector<double> &init_coeffs,
        ...
        double limit = 1.0;
        TimeKeeper tk(limit); // 1 ミリ秒だけ焼きなましに使う（時間制限）
        int iter = 0;
        ...
        while (true) {
            ...
            if (input.T > 20000 and mode == 0) {
                for (auto &c : tmp_coeffs) {
                    c *= 1.1;
                }
            } else if (mode == 1) {
                for (auto &c : tmp_coeffs) {
                ...
                // 4) 更新候補をつくる
                //    - 元 hill climb と同様に 1 つの係数をランダムに +-
                //    乱数で振る
                //    - ただし「非ゼロ本数 cnt」が max_colors
                //    を超えないように制約
                ...
                // 「増加方向 diff>0 なのに max_colors 本まで達していて、かつ
                // idx が 0 ならスキップ」
    ...
    void update(Color target, const vector<double> &coefficients,
        ...
        rep(i, actions.size()) {
            ...
            cerr << '\n';
            // assert(tmp.size() != 5 or (tmp[1] != tmp[3] or tmp[2] !=
            // tmp[4]));
            dump(tmp);
