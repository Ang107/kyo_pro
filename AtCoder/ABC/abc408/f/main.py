...
class SegTree:
    ...
    def __str__(self) -> str:
        return f"SegTree: {self._d[self._size:self._size + self._n]}"

    def __iter__(self) -> typing.Iterator[typing.Any]:
        return iter(self._d[self._size : self._size + self._n])
...
st = SegTree(lambda a, b: max(a, b), 0, n)

tmp = [(i, j) for i, j in enumerate(h)]
tmp.sort(key=lambda x: x[1], reverse=True)
sousa = deque()
for i, a in tmp:
    res = st.prod(max(0, i - r), min(i + r + 1, n)) + 1
    sousa.append((i, res))
    # print(i, a, sousa)
    if len(sousa) >= d:
        i, res = sousa.popleft()
