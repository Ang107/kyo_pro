    ...
    if next_g != -1 and time.get_passed_time() < 2.3:
        ...
        if not flag:
            ...
            while deq.len() > 0:
                var v = deq.pop()
                for next in input.G[v]:
                    if next in result and visited[next] == -2:
                        visited[next] = v
                        deq.add(next)
            var now = g
            while visited[now] != -1:
...
proc beamsearch(input: Input, output: var Output, time: Time) =
    ...
    var
        cands_array = newSeq[array[N, Cand]](N+1)
    ...
    for i in 0..N:
        for j in 0..<N:
            cands_array[i][j] = new_cand(State(), Action())
            cands_array[i][j].state.cost = INF64
        ...
        for (s, _, now) in next[0..<min(next.len(), beam_width)]:
            ...
            if now.change_signal == -1:
                ...
                for i in get_next_cand(g, cand.state.b):
                    if cand.state.cost < cands_array[turn+1][i].state.cost:
                ...
                for l in countdown(min(input.LA-ai, input.LB), 1, 1):
                    ...
                    if can_visit(frm, g, nblues):
                        for i in get_next_cand(g, nblues):
                            if cand.state.cost + now.times < cands_array[turn+1][i].state.cost:
                                cands_array[turn+1][i] = new_cand(
        ...
        while deq.len() > 0 and not fin:
            ...
            for next in input.G[v]:
                if next == s:
                    fin = true
                    frm[next] = v
                elif next in blues and frm[next] == -2:
                    frm[next] = v
                    deq.add(next)
        result.add(frm[s])
    ...
    for i in 0..<N:
        ...
        if t2 != -1:
            ...
            var now = change_times[t1][t2][index][1]
...
proc solve(self: Solver, input: var Input, output: var Output, time: Time): bool =
...
# -----------------------------------------------------------------------
proc get_score(actions: seq[(string, int, int, int)]): int =
    for i in actions:
        if i[0] == "s":
            result += 1
    return result
...
proc main() =
    var
        ...
        best_score = INF64
        best_A: seq[int]
        best_actions: seq[(string, int, int, int)]
    var backup_G = input.G
    for i in 0..<N:
    ...
    while true:
        GLOBALCNT += 1
        var
            ok = solver.solve(input, output, time)
            new_score = get_score(output.actions)
        if ok and best_score > new_score:
            best_score = new_score
            best_A = output.A
            best_actions = output.actions
        var now_time = time.get_passed_time()
        var average_time = now_time / GLOBALCNT
        if average_time + now_time > 2.3:
            break
        output.actions.setLen(0)
        output.A.setLen(0)
        can_go.fill(false)
        input.G = backup_G
