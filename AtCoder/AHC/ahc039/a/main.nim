...
proc solve(self: var Solver, output: var Output) =
    var
        ...
        nblock_len = 1000
        nblock_num = 100000 // nblock_len + 2
        nscore = newSeqWith(nblock_num+4, newseq[int](nblock_num+4))
        nb = newSeqWith(nblock_num+4, newseq[int](nblock_num+4))
    for (x, y) in self.input.sa:
        score[x//block_len+1][y//block_len+1] += 1
        nscore[x//nblock_len+2][y//nblock_len+2] += 1
    ...
    var
        ...
        end_temp = 0
        temp: float
    proc calc_temp(start_temp, end_temp: int, elapsed_time, TIME_LIMIT: float): float =
        # stdout.writeLine(elapsed_time, " ", now_score)
        return start_temp + (end_temp - start_temp) * elapsed_time / TIME_LIMIT
    var changed = false
    while true:
        if cnt mod 1000 == 0:
            var now = self.time.get_passed_time()
            if not changed and now > 1.5:
                changed = true
                for i in 0..<block_num+2:
                    for j in 0..<block_num+2:
                        if b[i][j] == 1:
                            chmin(min_x, i*2)
                            chmax(max_x, i*2+1)
                            chmin(min_y, j*2)
            ...
            if now > 1.95:
                break
            if changed:
                temp = calc_temp(start_temp, end_temp, now, TIME_LIMIT)
            else:
                temp = calc_temp(start_temp, end_temp, now, 1.5)
