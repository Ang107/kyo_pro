...
# 多様性が保たれるように長さ2LBの経路をたくさん作る。
# 長さがlimit以上同じものは追加しないようにする。limitは要検証 LB/2ぐらい？
# proc random_make_edge(input: Input, output: Output): seq[int] =
#     discard
#     var roots: array[N, array[N, (int, int)]]
#     for i in 0..<N:
...
proc tsp(input: Input, output: Output, time: Time): seq[int] =
    ...
    var diss: array[N, seq[(int, int, int)]]
    for i in 0..<N:
        diss[i] = newSeqOfCap[(int, int, int)](N)
        for j in 0..<N:
            diss[i].add((roots[i][j][0], roots[i][j][1], j))
        diss[i].sort(proc (a, b: (int, int, int)): int = cmp(a[0], b[0]))
        ...
        while updated:
            ...
            for i in 1..<order.len()-1:
                for j in i+1..<order.len():
                    var prv = roots[order[i-1]][order[i]][0] + roots[order[j]][
                            order[(j+1)%order.len()]][0]
                    var nxt = roots[order[i-1]][order[j]][0] + roots[order[i]][
                            order[(j+1)%order.len()]][0]
                    if prv > nxt:
    ...
    #     var
    #         each_len = (601) // split_num + int((601)%split_num != 0)
    #         A = newSeqOfCap[int](input.LA)
    #     echo "#each)len", each_len
    #     for i in 0..<split_num:
    #         if i != 0:
    #             for j in bfs(input.TS[i*each_len-1], input.TS[i*each_len])[0 ..< ^1]:
    #                 A.add(j)
    #         var target_seq = input.TS[i * each_len ..<
    #                                 min((i+1)*each_len, input.TS.len())]
    #         for j in in_tsp(target_seq, split_num):
    #             A.add(j)
    #     if A.len() <= input.LA:
    #         result = A
