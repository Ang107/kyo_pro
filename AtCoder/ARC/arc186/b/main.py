...
n = II()
a = LMII()
more_big = [[] for _ in range(n)]
# more_small = [[] for _ in range(n)]
# in_ = [[] for _ in range(n)]
for i, j in enumerate(a, start=1):
    if i - 1 > j:
        more_big[i - 1].append((j, i - 1))
    if j != 0:
        more_big[j - 1].append((i - 1, i))
print(more_big)


# more_big = LazySegTree(op, e, mapping, composition, id_, [0] * n)
# more_small = LazySegTree(op, e, mapping, composition, id_, [0] * n)
# for i, j in enumerate(a, start=1):
#     if i - 1 > j:
#         more_big.set(i - 1, more_big.prod(j, i - 1) + (i - j - 1))
#         more_small.apply(j, i - 1, more_small.get(i - 1))
